...
#重置superlucky和superluckysss
def rest_superlucky():
	superlucky=[]
	superluckysss=[]
	with open ("superluckysss.json","w")as write_file:
		write_file.truncate()
		print("清空superluckysss.json成功")
	load_s_and_n()
	print("加载普通池子和up池子成功")
...
def load_s_and_n():
	global superlucky
	global superluckysss
	global nolucky
	try:
		with open ("superluckysss.json","r")as read_file:
			superlucky=json.load(read_file)
			print("载入superluckysss.json成功")
	except:
		superlucky=[]
		print("未发现superluckysss.json")
	nolucky=list(set(names)-set(superlucky))

#添加到将superlucky+lucky_dog添加到superluckysss
#刷新nolucky
def add_to_superluckysss():
	global superlucky
	global superluckysss
	superluckysss=list(set(superlucky)|set(lucky_dogs))
	nolucky=list(set(names)-set(superluckysss))
...
def save_superlucky():
	with open("superluckysss.json","w") as write_file:
		json.dump(superluckysss,write_file)
		print("保存superluckysss.json成功")
